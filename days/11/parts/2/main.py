...
empty_rows = {i for i, line in enumerate(lines) if line.strip() == ""}
empty_cols = {i for i, line in enumerate(zip(*lines)) if "".join(line).strip() == ""}
...
for i in range(len(galaxies)):
    for j in range(i + 1, len(galaxies)):
        ...
        count = (bx - ax) + len(empty_cols & set(range(ax, bx + 1))) * (1000000 - 1)
        count += (by - ay) + len(empty_rows & set(range(ay, by + 1))) * (1000000 - 1)
